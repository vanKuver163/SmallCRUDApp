...
export default function App() {
  ...
  function deletePost(postId){
    const url = `${Constants.API_URL_GET_ALL_POSTS}/${postId}`;
    ...
    <div className="container">
      <div className="row min-vh-100">
        <div className="col d-flex flex-column justify-content-center align-items-center">
          ...
          {
            (posts.length > 0 && showingCreateNewPostForm === false && postCurrentlyBeingUpdated === null) && renderPostsTabel()
          }
          {
  ...
  function renderPostsTabel() {
    return (
      <div className="tabel-responsive mt-5">
            ...
            {posts.map((post) => (
              <tr key={post.postId}>
                ...
                <td>
                  ...
                  <button onClick={() => { if(window.confirm(`Are you sure want to delete the post titled "${post.title}"?`)) deletePost(post.postId)}} className="btn btn-secondary btn-lg">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  function onPostUpdated(updatedPost){
    ...
    if(index !== -1){
      ...
      setPosts(postsCopy);
      alert(`Post successfully updated. After clicking Ok, look for the post with the title "${updatedPost.title}" in the tabel delow to see the update.`);
  }
  ...
  function onPostDeleted(deletedPostPostId){    
    ...
    if(index !== -1){
      ...
      setPosts(postsCopy);
      alert('Post successfully deleted. After clicking OK.');
  }
